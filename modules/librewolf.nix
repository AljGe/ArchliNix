{ config, lib, pkgs, ... }:
let
  mkEngine = { template, params ? [ ], icon ? null }:
    let url = { inherit template params; };
    in lib.filterAttrs (k: v: v != null) {
      urls = [ url ];
      inherit icon;
    };
in
{
  programs.librewolf = {
    ...
    profiles.default = {
      ...
      containers = {
        Personal = { id = 1; icon = "fingerprint"; color = "toolbar"; };
        Work = { id = 2; icon = "briefcase"; color = "orange"; };
        Shopping = { id = 3; icon = "cart"; color = "pink"; };
      };
      containersForce = true;
      extensions = {
        packages = with pkgs.nur.repos.rycee.firefox-addons; [
          ublock-origin clearurls libredirect localcdn
          sidebery tree-style-tab sponsorblock
          i-dont-care-about-cookies
          darkreader proton-pass
        ];
        force = true;
      };
        userChrome = "
